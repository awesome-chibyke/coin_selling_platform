@php $titleDescription = 'Referal Bonus'
@endphp
                ...
                <div class="card">
                  <div class="card-header">
                    <div class="row" style="width:100%;">
                        <h4 class="col-12 col-sm-8">My Referals Earnings</h4>
                        ...
                        <table class="table table-striped" id="table-1">
                          <thead>
                            <tr>
                              ...
                              <th class="text-center">Referred Name</th>
                          ...
                          <tbody>
                            ...
                            @php $counter = 1;  @endphp
                                @foreach ($referals as $each_referal)
                                    <tr>
                                        <td class="text-center" scope="col">{{$counter}}</td>
                                        <td class="text-center sorting_1">
                                            <input type="checkbox" class="smallCheckBox" value="{{$each_referal->unique_id}}">
                                        </td>
                                        <td class="text-center">{{ $each_referal->referredObject->name }}</td>
                                        <td class="text-center">{{ $each_referal->referrerObject->name }}</td>
                                        <td class="text-center">$ {{ $each_referal->paymentObject->amount_in_usd }}</td>
                                        <td class="text-center">{{ $each_referal->paymentObject->local_currency }} {{ round($each_referal->amount) }}</td>
                                        @php $statusObject = $each_referal->returnStatus(); @endphp
                                        <td class="text-center"><span class="btn btn-{{ $statusObject->class }}">{{ strtoupper($each_referal->status) }}</span></td>
                                        <td class="text-center">{{ $each_referal->created_at }}</td>
                                        {{-- <td class="text-center action_holder" data-url="{{ route('manage-user-level', [$each_referal->unique_id]) }}">
                            ...
                            @else
                                <tr class="odd">
                                    <td colspan="8" class="dataTables_empty text-center" valign="top">No matching records found</td>
                                </tr>
                            @endif
                          </tbody>
                        </table>
